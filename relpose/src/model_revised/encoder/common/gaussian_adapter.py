...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        c2w_rotations = extrinsics[..., :3, :3][:, 0:1] if not use_pred_cams else pred_cams[..., :3, :3] #! camera pose estimation 
        covariances = c2w_rotations @ covariances @ c2w_rotations.transpose(-1, -2)

        # Compute Gaussian means.
        origins, directions = get_world_rays(coordinates, extrinsics[:, 0:1], intrinsics[:, 0:1])
