    ...
    def __init__(
        ...
        **kwargs,
        ...
        if wo_cost_volume_refine:
            ...
            self.corr_refine_net = nn.Sequential(*modules) #* 2D UNet
        ...
        if self.predict_only_canonical:
            self.proj_feature = nn.Conv2d(
                upsample_out_channels*num_views, depth_unet_feat_dim, 3, 1, 1
            )
        else:
            self.proj_feature = nn.Conv2d(
                upsample_out_channels, depth_unet_feat_dim, 3, 1, 1
        ...
        input_channels = depth_unet_feat_dim + 1 + 1
        ...
        gau_in = gau_in + 3 * num_views if self.predict_only_canonical else gau_in + 3
    ...
    def forward(
        ...
        feat_comb_lists, intr_curr, pose_curr_lists, disp_candi_curr = (
            prepare_feat_proj_data_lists(
                features,
                intrinsics,
                extrinsics if not use_pred_cams else pred_cams,
                near,
                far,
        ...
        if self.wo_cost_volume:
            ...
            for feat10, pose_curr in zip(feat_comb_lists[1:], pose_curr_lists):
                ...
                raw_correlation_in = (feat01.unsqueeze(2) * feat01_warped).sum(
                    ...
                    c**0.5 #! correlation modeling (paper Eq.3)
                )  # [vB, D, H, W]
                raw_correlation_in_lists.append(raw_correlation_in) #* reference와의 유사도를 계산
            # average all cost volumes
            raw_correlation_in = torch.mean( #* reference와의 유사도를 평균
        ...
        # calculate mean and variance of the cost volume (raw_correlation_in: (vxb, d, h', w'))
        
        if predict_only_canonical:
            raw_correlation_in = rearrange(raw_correlation_in, "(b v) d h w -> b v d h w", v=v)
            raw_correlation_in_mean = raw_correlation_in.mean(dim=1, keepdim=False) # (b, 1, d, h, w)
            raw_correlation_in_var = raw_correlation_in.var(dim=1, keepdim=False) # (b, 1, d, h, w)

            raw_correlation_in = torch.cat((raw_correlation_in_mean, raw_correlation_in_var), dim=1) # (b, 2*d, h, w)

            raw_correlation = self.corr_refine_net(raw_correlation_in)  # (vb d h w) #* 2D U-Net
        ...
        if predict_only_canonical: # d
            ...
            img_colors = rearrange(extra_info['images'], "(b v) c h w -> b (v c) h w", v=v)
            
            v = 1
        else:
            img_colors = extra_info['images']
            
            
        proj_feature = self.proj_feature(proj_feat_in_fullres)
        refine_out = self.refine_unet(torch.cat(
            (img_colors, proj_feature, fullres_disps, pdf_max), dim=1
        ))

        # gaussians head
        raw_gaussians_in = [refine_out,
                            img_colors, proj_feat_in_fullres]
        
        raw_gaussians_in = torch.cat(raw_gaussians_in, dim=1) #* shape: (2, 163, 128, 160)
        raw_gaussians = self.to_gaussians(raw_gaussians_in) #* 84 parameters
        raw_gaussians = rearrange(
            raw_gaussians, "(v b) c h w -> b v (h w) c", v=v, b=b #! v=v
        )
