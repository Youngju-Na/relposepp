    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                extrinsics_chosen = extrinsics[all_indices].clone()
                
                                # Extract the first extrinsic (rotation and translation)
                def transform_extrinsics(extrinsics):
                    ref_inv = torch.inverse(extrinsics[0])
                    return torch.einsum('ij,njk->nik', ref_inv, extrinsics)

                extrinsics_chosen = transform_extrinsics(extrinsics_chosen)
                
                extrinsics[all_indices] = extrinsics_chosen
