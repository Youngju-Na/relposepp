...
def evaluate_category_translation(
    ...
    **kwargs,
    ...
    for i, metadata in enumerate(iterable):
        ...
        # translation = torch.linalg.norm(R_pred - R_gt_rel, dim=-1)
        norm_pred = T_pred# / torch.linalg.norm(T_pred, dim = -1).unsqueeze(-1)
        norm_gt =  T_gt# / torch.linalg.norm(T_gt, dim =-1).unsqueeze(-1)
        # cosine_similarity_0 = torch.dot(norm_pred[0], norm_gt[0])
        cosine_similarity_1 = torch.dot(norm_pred[1], norm_gt[1])
        cosine_similarity_2 = torch.dot(norm_pred[2], norm_gt[2])
        ...
        angle_radians_2 = torch.arccos(torch.clip(cosine_similarity_2, -1.0,1.0))
        angular_means = ( angle_radians_1 + angle_radians_2) / 2
        angular_errors.append(angular_means.detach().cpu().numpy())
    ...
    print(np.average(np.array(angular_errors))) # 0.013313462
